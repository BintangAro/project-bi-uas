...
def transform():
    ...
    kontribusi = df.groupby(["id_region", "channel_key", "nama_channel"]).agg({"qty": "sum"}).reset_index()
    kontribusi.rename(columns={"qty": "total_qty"}, inplace=True)
    kontribusi.to_csv(OUTPUT_KONTRIBUSI, index=False)
